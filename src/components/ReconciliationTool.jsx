  ...
  return (
    <div className="space-y-6 font-sans max-w-full">
      ...
      <div className="glass-panel p-4 sm:p-5 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-sm sm:text-base font-extrabold text-white flex items-center gap-2">
            <GitCompare className="w-5 h-5 text-emerald-400 shrink-0" />
        ...
        <button
          ...
          className="px-3.5 py-2 bg-slate-800 hover:bg-slate-700 text-emerald-400 border border-slate-700 text-xs font-bold rounded-xl transition-all flex items-center justify-center gap-1.5 min-h-[40px]"
        >
      ...
      <div className="glass-panel p-5 sm:p-6 rounded-2xl space-y-5">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-950 border border-slate-800 p-4 rounded-xl space-y-2">
            ...
            <input
              ...
              className="text-xs text-slate-400 w-full file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-slate-800 file:text-emerald-400 cursor-pointer"
            />
            {fileA && <div className="text-[11px] text-emerald-400 font-mono font-bold break-all">Loaded: {fileA.name}</div>}
          </div>
          ...
          <div className="bg-slate-950 border border-slate-800 p-4 rounded-xl space-y-2">
            ...
            <input
              ...
              className="text-xs text-slate-400 w-full file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-slate-800 file:text-cyan-400 cursor-pointer"
            />
        ...
        {/* Tolerance Controls */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-3 border-t border-slate-800/80 items-center">
          <div>
            <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">
              Amount Tolerance: <strong className="text-emerald-400 font-mono">± ₹{toleranceAmount}</strong>
            </label>
            <input
              type="range"
              min="0"
          ...
          <div>
            <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">
              Date Window Tolerance: <strong className="text-cyan-400 font-mono">± {toleranceDays} Days</strong>
            </label>
            <input
              type="range"
              min="0"
          ...
          <button
            ...
            className="w-full py-3 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 disabled:opacity-50 text-slate-950 font-extrabold text-xs rounded-xl shadow-lg transition-all flex items-center justify-center gap-1.5 min-h-[44px]"
          >
        ...
        <div className="glass-panel p-5 sm:p-6 rounded-2xl space-y-5 animate-fadeIn">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 border-b border-slate-800 pb-4">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 w-full md:w-auto">
              ...
              <button
                ...
                className={`p-3 rounded-xl border text-left transition-all ${
                  activeBucket === 'matched' ? 'bg-emerald-500/10 border-emerald-500/50 text-emerald-400' : 'bg-slate-900 border-slate-800 text-slate-400'
                }`}
              >
                <span className="text-[10px] font-bold uppercase tracking-wider block">Bucket 1: Matched</span>
                <span className="text-lg sm:text-xl font-bold font-mono text-white">{matchResult.matched_count}</span>
              </button>

              <button
                onClick={() => setActiveBucket('unmatched_a')}
                className={`p-3 rounded-xl border text-left transition-all ${
                  activeBucket === 'unmatched_a' ? 'bg-rose-500/10 border-rose-500/50 text-rose-400' : 'bg-slate-900 border-slate-800 text-slate-400'
                }`}
              >
                <span className="text-[10px] font-bold uppercase tracking-wider block">Bucket 2: Unmatched A</span>
                <span className="text-lg sm:text-xl font-bold font-mono text-white">{matchResult.unmatched_a_count}</span>
              </button>

              <button
                onClick={() => setActiveBucket('unmatched_b')}
                className={`p-3 rounded-xl border text-left transition-all ${
                  activeBucket === 'unmatched_b' ? 'bg-amber-500/10 border-amber-500/50 text-amber-400' : 'bg-slate-900 border-slate-800 text-slate-400'
                }`}
              >
                <span className="text-[10px] font-bold uppercase tracking-wider block">Bucket 3: Unmatched B</span>
                <span className="text-lg sm:text-xl font-bold font-mono text-white">{matchResult.unmatched_b_count}</span>
              </button>
            ...
            <button
              ...
              className="w-full md:w-auto px-4 py-2.5 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-extrabold text-xs rounded-xl transition-all shadow-md flex items-center justify-center gap-1.5 min-h-[42px]"
            >
          ...
          <div className="table-responsive-container">
                ...
                <tbody className="divide-y divide-slate-800/60 font-sans">
                  {matchResult.matched.map((m, idx) => (
                    <tr key={idx} className="hover:bg-slate-900/60">
                      ...
                      <td className="py-3 px-3 text-right font-mono text-slate-200">₹{(m.amount_a||0).toLocaleString()}</td>
                      <td className="py-3 px-3 text-right font-mono text-slate-200">₹{(m.amount_b||0).toLocaleString()}</td>
                      <td className="py-3 px-3 text-right font-mono text-amber-400 font-bold">₹{m.amount_diff}</td>
            ...
            {activeBucket === 'unmatched_a' && (
              <div className="space-y-2">
                <span className="text-xs font-bold text-rose-400 block font-mono mb-2">Rows present in File A but MISSING in File B:</span>
                <pre className="bg-slate-950 border border-slate-800 p-4 rounded-xl text-xs font-mono text-rose-300 max-h-60 overflow-y-auto">
            ...
            {activeBucket === 'unmatched_b' && (
              <div className="space-y-2">
                <span className="text-xs font-bold text-amber-400 block font-mono mb-2">Rows present in File B but MISSING in File A:</span>
                <pre className="bg-slate-950 border border-slate-800 p-4 rounded-xl text-xs font-mono text-amber-300 max-h-60 overflow-y-auto">
