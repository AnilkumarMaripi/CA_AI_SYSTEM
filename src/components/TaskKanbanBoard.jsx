...
export default function TaskKanbanBoard({ tasks = [], users = [], onAssignTask, onUpdateStage }) {
  ...
  const stages = [
    { id: 'Pending', label: 'To Do (Pending)', color: 'border-slate-800 bg-slate-900 text-slate-300' },
    { id: 'In Progress', label: 'In Progress', color: 'border-cyan-500/40 bg-cyan-500/10 text-cyan-400' },
    { id: 'Review', label: 'Under Review', color: 'border-amber-500/40 bg-amber-500/10 text-amber-400' },
    { id: 'Filed', label: 'Done (Filed)', color: 'border-emerald-500/40 bg-emerald-500/10 text-emerald-400' }
  ];
    ...
    <div className="space-y-6 font-sans max-w-full">
      ...
      <div className="glass-panel p-4 sm:p-5 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-2 w-full md:w-auto">
          <span className="text-xs text-slate-400 font-bold uppercase tracking-wider shrink-0">Filter Staff:</span>
          <select
            value={selectedUserFilter}
            onChange={(e) => setSelectedUserFilter(e.target.value)}
            className="w-full md:w-auto bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs font-bold text-white focus:outline-none focus:border-emerald-500 min-h-[38px]"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {stages.map(stage => {
          ...
          return (
            <div key={stage.id} className="bg-slate-900/60 border border-slate-800 rounded-2xl p-4 space-y-3 flex flex-col min-h-[300px] sm:min-h-[500px]">
              
              {/* Column Header */}
              <div className={`p-2.5 rounded-xl border font-bold text-xs flex items-center justify-between font-mono ${stage.color}`}>
                <span>{stage.label}</span>
                <span className="px-2 py-0.5 rounded bg-slate-950 text-white text-[11px]">
                  {columnTasks.length}
                </span>
              </div>
              ...
              <div className="space-y-3 flex-1 overflow-y-auto max-h-[500px] sm:max-h-[600px] pr-1">
                {columnTasks.map(task => (
                  <div key={task.id} className="bg-slate-950 border border-slate-800 p-4 rounded-xl space-y-3 hover:border-slate-700 transition-all shadow-md">
                    ...
                    <div>
                      ...
                      <span className="text-[10px] text-slate-400 font-mono block mt-1">Client: {task.client_name}</span>
                    </div>
                    ...
                    <div className="pt-2 border-t border-slate-800/80 flex items-center justify-between gap-1 text-[10px]">
                      ...
                      <select
                        ...
                        className="bg-slate-900 border border-slate-800 text-slate-300 rounded px-2 py-1 text-[10px] font-mono focus:outline-none max-w-[120px] min-h-[30px]"
                      >
                      ...
                      <div className="flex items-center space-x-1">
                        {stage.id !== 'Filed' && (
                          <button
                            ...
                            className="px-2.5 py-1 bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 rounded font-bold transition-all min-h-[30px] flex items-center gap-1"
